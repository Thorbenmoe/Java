...
public class maximum
{
	...
	{	
		...
		//Arrays erstellen
		System.out.println("Wie viele Arrays möchten sie einlesen lassen?\n");
		System.out.println("Eingabe: ");
		int arrayMenge=scanner.nextInt();
		int[][] matrix=new int[arrayMenge][0];
		for(int i=0; i<=arrayMenge-1; i++)
		{
			System.out.println("***Array "+(i+1)+ " Reichweite bzw Länge eingeben.***\n");
			reichWeite=scanner.nextInt();
			System.out.println("***Array "+(i+1)+" ZahlenRaum bzw Länge eingeben.***\n");
		...
		for(int j=0; j<=arrayMenge-1; j++)
		{	
			System.out.println("Array "+(j+1)+": " + Arrays.toString(matrix[j]));
			minMaxAusgabe(matrix[j]);
		}
		//aufge4 4 P T3
//		shuffle(array,arrayZwei);
		//Test für rekursive Funktion für fakultätsbestimmung für den wert von reichweite.
		//vorbehalten
		reichWeite=rekursivFunction(arrayMenge);
		System.out.println("Fakultät von der Menge der Arrays: "+arrayMenge);
		//
		}
}
